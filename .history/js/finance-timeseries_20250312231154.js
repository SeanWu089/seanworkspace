...
function createCustomSelect(originalSelect) {
    ...
    defaultOption.className = 'option-item default-option';
    ...
    Array.from(originalSelect.options).forEach(option => {
        if (option.value && !uniqueFiles.has(option.textContent)) {
            uniqueFiles.add(option.textContent);
    ...
    selectedOption.addEventListener('click', (e) => {
        ...
        selectedOption.querySelector('.arrow').classList.toggle('rotate');
    });

    // 点击外部区域时关闭下拉列表
    document.addEventListener('click', () => {
        customSelect.classList.remove('open');
        selectedOption.querySelector('.arrow')?.classList.remove('rotate');
    });
    ...
    try {
        ...
        data.forEach(file => {
            if (supportedExtensions.some(ext => file.name.toLowerCase().endsWith(ext))) {
                const baseName = file.name.replace(/^\d+_/, ''); // 移除时间戳前缀
        ...
        // 对每个文件名，只保留最新的版本
        const uniqueFiles = Array.from(fileMap.values()).map(files => {
            // 按创建时间排序，取最新的
