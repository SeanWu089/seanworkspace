    ...
    button.addEventListener('click', async function() {
        try {
            ...
            if (!window.supabaseClient) {
                console.warn('Supabase client not initialized, using local file selection');
                // 使用本地文件选择作为备选方案
            ...
            const session = await checkUserAuth();
            if (!session) {
                console.warn('User not logged in, using local file selection');
                // 使用本地文件选择作为备选方案
                const fileInput = document.getElementById('file-input');
                if (fileInput) {
                    fileInput.click();
