...
function getVariableTypeIcon(variable, sampleValue) {
    ...
    if (variable.toLowerCase().includes('date') || 
        ...
        variable.toLowerCase().includes('year')) {
        return '<i class="fas fa-calendar-alt variable-icon date"></i>';
    } else if (typeof sampleValue === 'number') {
        return '<i class="fas fa-chart-line variable-icon continuous"></i>';
    } else if (sampleValue && sampleValue.length > 20) {
        return '<i class="fas fa-align-left variable-icon text"></i>';
    } else {
        return '<i class="fas fa-list-ul variable-icon categorical"></i>';
    }
}
