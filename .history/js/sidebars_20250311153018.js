...
function updatePreviewTable(data, rawData = null) {
    ...
    if (!data || data.length === 0) {
        console.log('No data provided, checking localStorage');
        // 从 localStorage 获取之前保存的预览数据
        const savedPreview = localStorage.getItem('previewData');
        if (savedPreview) {
            try {
                console.log('Found saved preview data');
            ...
            console.log('No saved preview data found');
            previewEmptyState.style.display = 'block';
            previewContent.style.display = 'none';
            if (fullPreviewContent) fullPreviewContent.style.display = 'none';
            return;
    ...
    if (tbody) {
        console.log('Updating preview table body');
        ...
        localStorage.setItem('previewData', JSON.stringify(saveData));
        ...
        if (rawData && fullPreviewContent) {
            console.log('Updating full preview table');
            updateFullPreviewTable(rawData);
        } else {
            console.log('No raw data available for full preview');
        }
    } else {
...
function updateFullPreviewTable(rawData) {
    try {
        console.log('Updating full preview table with raw data');
        
        const fullPreviewContent = document.getElementById('full-preview-content');
        if (!fullPreviewContent) {
            console.error('Full preview content element not found');
            return;
        }
        
        if (!rawData) {
            console.error('No raw data provided for full preview');
            return;
        }
        
        const fullPreviewTable = document.getElementById('full-preview-table');
        if (!fullPreviewTable) {
            console.error('Full preview table element not found');
            return;
        }
        ...
        if (!rawData.headers || !Array.isArray(rawData.headers)) {
            console.error('Invalid headers in raw data:', rawData.headers);
            return;
        }
        
        console.log('Creating table header with', rawData.headers.length, 'columns');
        ...
        if (!rawData.rows || !Array.isArray(rawData.rows)) {
            console.error('Invalid rows in raw data');
            return;
        }
        
        console.log('Creating table body with', rawData.rows.length, 'rows');
        
        // 限制显示的行数，以避免性能问题
        const maxRows = 100;
        const rowsToShow = rawData.rows.slice(0, maxRows);
        ...
        tbody.innerHTML = rowsToShow.map(row => {
            ...
            return `<tr>${row.map(cell => `<td>${cell !== undefined && cell !== null ? cell : ''}</td>`).join('')}</tr>`;
        }).join('');
        
        if (rawData.rows.length > maxRows) {
            console.log(`Showing only ${maxRows} of ${rawData.rows.length} rows`);
            tbody.innerHTML += `<tr><td colspan="${rawData.headers.length}" class="more-rows-message">显示 ${maxRows} 行中的 ${rawData.rows.length} 行</td></tr>`;
        }
        
        // 默认隐藏完整预览
        fullPreviewContent.style.display = 'none';
        console.log('Full preview table updated successfully');
    } catch (error) {
        console.error('Error updating full preview table:', error);
    }
}
...
function toggleSidebar(side, forceShow = null) {
    ...
    if (side === 'right' && sidebar.classList.contains('show')) {
        ...
        // 确保两个元素都存在
        if (previewContent && fullPreviewContent) {
            // 检查预览内容是否显示
            const isPreviewVisible = previewContent.style.display !== 'none';
            
            // 如果是点击切换按钮且不是强制显示，且预览内容可见，则切换完整预览
            if (forceShow === null && isPreviewVisible) {
                toggleFullPreview();
            }
        }
    }
...
function toggleFullPreview() {
    try {
        ...
        if (!rightSidebar || !previewContent || !fullPreviewContent) {
            console.error('Missing required elements for full preview toggle');
            return;
        }
        
        console.log('Toggling full preview');
        
        // 检查完整预览是否已显示
        const isFullPreviewVisible = fullPreviewContent.style.display === 'block';
        
        if (isFullPreviewVisible) {
            // 如果完整预览已显示，则隐藏它并恢复边栏宽度
            console.log('Hiding full preview');
            fullPreviewContent.style.display = 'none';
            previewContent.style.display = 'block';
            rightSidebar.style.width = '300px';
        } else {
            // 否则显示完整预览并扩展边栏宽度
            console.log('Showing full preview');
            fullPreviewContent.style.display = 'block';
            previewContent.style.display = 'none';
            rightSidebar.style.width = '66%';
        }
    } catch (error) {
        console.error('Error toggling full preview:', error);
    }
}
...
function initRightSidebar(toggle, sidebar) {
    toggle.onclick = function() {
        ...
        if (isVisible) {
            ...
            // 如果预览内容可见，则切换到完整预览
            if (previewContent && previewContent.style.display === 'block' && fullPreviewContent) {
                console.log('Switching to full preview');
                toggleFullPreview();
            } else if (fullPreviewContent && fullPreviewContent.style.display === 'block') {
                // 如果完整预览已显示，则关闭边栏
    ...
    fileInput.addEventListener('change', async function(event) {
        if (this.files && this.files.length > 0) {
            try {
                ...
                // 清除文件输入，以便下次选择同一文件时也能触发change事件
                this.value = '';
            } catch (error) {
