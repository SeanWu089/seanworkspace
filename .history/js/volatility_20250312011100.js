  ...
  function renderForecast(results) {
    ...
    for (const ticker in results) {
      ...
      for (let i = 0; i < data.forecast.length; i++) {
        ...
        volatilityCell.textContent = data.forecast[i].volatility.toFixed(4);
        row.appendChild(volatilityCell);
        
        const varCell = document.createElement('td');
        varCell.textContent = data.forecast[i].var.toFixed(4);
        row.appendChild(varCell);
        
        const esCell = document.createElement('td');
        esCell.textContent = data.forecast[i].es.toFixed(4);
      ...
      forecastResults.appendChild(forecastTable);
    }
  }
});